...
handlers._shops.get = (data,callback)=>{
    ...
    var nextPageToken = typeof(data.queryStringObject.nextPageToken) == 'string' ? data.queryStringObject.nextPageToken : false;
    console.log(lat);
    console.log(lng);
    _data.read('users' , {accessToken : token},res=>{
        if(res){
            ...
            if( (lat && lng) || nextPageToken){
                ...
                var targetUrl = nextPageToken ? apiPath+'pagetoken='+nextPageToken+'&key='+config.placesApiKey : apiPath+'location='+lat+','+lng+'&rankby=distance&type=shopping_mall&key='+config.placesApiKey ;

                fetch(targetUrl)
                .then(blob => blob.json())
                .then(data => {
                    var dislikedShops = userInfo.disliked.map(shop=>shop.shopId);
                    var shopsList = data.results.filter(shop=>dislikedShops.indexOf(shop.place_id)==-1);
                    shopsList = shopsList.map(e=>{
                        var shop = {};
