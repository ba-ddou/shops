    ...
    if(data.shopObject){
        ...
        _data.read('users',{accessToken : token},res=>{
            if(res){
                ...
                var impressions = typeof(res[0][data.impression]) == 'object' ? res[0][data.impression] : [];

                //check if the shop already exists in the database
                _data.read('shops',{shopId : data.shopObject.shopId},res=>{
                    if(res){
                        //store the shops users array and add this new users's _id
                        var users = res[0].users;
                        users.push(userId);
                        //update the shops document
                        _data.update('shops',{shopId : data.shopObject.shopId},{users : users},err=>{
                            if(!err){
                                //add the shops _id to the users impression array
                                impressions.push(res[0]._id);
                                
                                //update the users document
                                _data.update('users',{accessToken : token},{[data.impression] : impressions},err=>{
                                    if(!err){
                                        callback(200);
                                    }else{
                        ...
                        _data.create('shops',shopObject,(err,res)=>{
                            if(!err && res){
                                impressions.push(res.ops[0]._id);
                                //update the users document
                                _data.update('users',{accessToken : token},{[data.impression] : impressions},err=>{
                                    if(!err){
                                        callback(200);
